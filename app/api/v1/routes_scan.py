...
@router.post("/single")
async def scan_single(data: ScanRequest, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    """
    Escaneo COMPLETO individual
    """
    ports = normalize_ports(data.ports)

    result = await scan_worker.scan_single_host(ip=data.ip, ports=ports, db=db, user_id=user.id)

    open_only = {
        p: info for p, info in result["ports"].items() if info["status"] in ("open", "open|filtered", "filtered")
    }
...
async def scan_range_hosts(
    data: RangeScanRequest, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
    """
    ...
    results = await scan_worker.scan_network_range(cidr=data.network, ports=ports, db=db, user_id=user.id)

    return {"network": data.network, "ports_scanned": ports, "results": results}
    ...
    try:
        while True:
            ...
            if cmd == "single":
                ...
                result = await scan_worker.scan_single_host(ip=ip, ports=ports, on_update=send_update, user_id=user.id)
            ...
            # SCAN RANGE
                ...
                results = await scan_worker.scan_network_range(
                    cidr=cidr, ports=ports, on_update=send_update, user_id=user.id
                )

                await send_update({"type": "range_result", "cidr": cidr, "ports": ports, "results": results})
...
@router.get("/auto")
async def auto_scan_rest(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    """
    ...
    results = await scan_worker.scan_network_range(
        cidr=network_cidr, ports=list(range(1, 1024)), db=db, user_id=user.id
    )

    return {"network": network_cidr, "results": results}
...
async def auto_scan_ws(ws: WebSocket):
    ...
    try:
        ...
        results = await scan_worker.scan_network_range(
            cidr=network_cidr, ports=list(range(1, 1024)), on_update=send_update, user_id=user.id
        )

        await send_update({"type": "status", "message": "Escaneo completo", "results": results})
